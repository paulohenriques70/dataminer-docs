...
1. Specify the following database settings for the Cassandra nodes:
   ...
   - **DB server**: The IP addresses or hostnames of the nodes, separated by commas. Optionally, the default Cassandra port 9042 can be appended to each of the IP addresses or hostnames and only this port is supported (see [Configuring the IP network ports](xref:Configuring_the_IP_network_ports)). <!-- for the moment remove RN 34590 -->
